...
export const AdPlanInputSchema = z.object({
  ...
  urban_type: z
    ...
    .describe('The urban type of the target location.'),
  budget_level: z
...
export const AdPlanOutputSchema = z.object({
  kpis: z.object({
    estimated_reach: z
      .number()
      .describe('The estimated reach of the ad campaign.'),
    estimated_ctr_percent: z
      .number()
      .describe('The estimated click-through rate percentage.'),
    confidence_score_percent: z
      .number()
      .describe('The confidence score percentage.'),
    budget_level: z
      .enum(['low', 'medium', 'high'])
      .describe('The budget level for the campaign.'),
  }),
  campaign_overview: z.object({
    ...
    primary_objective: z
      .string()
      .describe('The primary objective of the campaign.'),
  }),
  geo_strategy: z.object({
    city: z.string().describe('The city where the ad will be targeted.'),
    primary_area: z
      .string()
      .describe('The primary area within the city to target.'),
    recommended_radius_km: z
      .number()
      .describe(
        'The recommended radius in kilometers around the primary area.'
      ),
    target_regions: z
      .array(
        z.object({
          ...
          priority: z
            ...
            .describe('The priority of targeting this area.'),
          reason: z
            .string()
            .describe('The reason why this area is a good target.'),
        })
      )
    ...
    time_windows: z
      .array(
        z.object({
          label: z
            .string()
            .describe('A label for the time window (e.g., Office commute).'),
          hours_local: z
            .string()
            .describe(
              'The hours of the time window in local time (e.g., 7:00-10:00).'
            ),
          reason: z
            .string()
            .describe('The reason why this time window is ideal.'),
        })
      )
      .describe(
        'A list of time windows to target with their label, hours, and reason.'
      ),
  }),
  audience_segments: z
    .array(
      z.object({
        ...
        description: z
          .string()
          .describe(
            'A description of who they are and what they care about (no sensitive traits).'
          ),
        age_range: z
          .string()
          .describe('The age range of the audience segment (e.g., 22-35).'),
        interests: z
          .array(z.string())
          .describe('A list of interests for the audience segment.'),
        preferred_channels: z
          .array(z.string())
          .describe(
            'The preferred advertising channels for the audience segment.'
          ),
      })
    )
    ...
    .array(
      z.object({
        ...
        ad_format: z
          .string()
          .describe(
            'The format of the ad (e.g., Mobile banner / Video / Native).'
          ),
        ...
        geo_context_hook: z
          .string()
          .describe(
            'A line referencing the local city/area or time of day.'
          ),
      })
    )
    .describe(
      'A list of creative ad suggestions tailored to specific audience segments.'
    ),
});
